...
export default function PreviewNav() {
  ...
  return (
    <nav className="flex items-center gap-xs flex-wrap mb-xxl">
      <Link
        to="/preview"
        className="type-caps tracking-[1.6px] text-[#979797] hover:text-black transition-colors duration-150 whitespace-nowrap"
      >
        Design System
      </Link>
      {LAYERS.map(({ path, label }) => (
        <span key={path} className="flex items-center gap-xs">
          <span className="type-caps text-[#979797]">/</span>
          <Link
            to={path}
            className={`type-caps tracking-[1.6px] transition-colors duration-150 whitespace-nowrap ${
              pathname === path
